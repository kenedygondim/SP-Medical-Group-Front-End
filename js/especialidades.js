...
async function fetchItems() {
    try {
        const getProfissionais = await fetch("http://localhost:8080/api/Medico/InfoDetalhesPrincipaisMedico");
        const getEspecialidades = await fetch("http://localhost:8080/api/TipoMedico/ListarTodos")
        ...
        if (especialidade !== null && nomeDoMedico === null && numCrm === null) { //apenas o o primeiro campo marcadao
            profissionais = items.filter(item => item.nomeTipoMedico === especialidade);
        }
        else if (especialidade === null && nomeDoMedico !== null && numCrm === null) { //apenas o segundo campo marcado
            profissionais = items.filter(item => item.nomeUsuario.includes(nomeDoMedico));
        }
            ...
            profissionais = items.filter(item => item.nomeTipoMedico === especialidade && item.nomeUsuario.includes(nomeDoMedico));
        }
        else if (especialidade !== null && nomeDoMedico === null && numCrm !== null) { //o primeiro e o terceiro campo marcados
            profissionais = items.filter(item => item.nomeTipoMedico === especialidade && item.crm === numCrm);
        }
        else if (especialidade === null && nomeDoMedico !== null && numCrm !== null) { //o segundo e o terceiro campo marcados
            profissionais = items.filter(item => item.nomeUsuario.includes(nomeDoMedico) && item.crm === numCrm);
        }
        else if (especialidade !== null && nomeDoMedico !== null && numCrm !== null) { //todos os campos marcados
            profissionais = items.filter(item => item.nomeTipoMedico === especialidade && item.nomeUsuario.includes(nomeDoMedico) && item.crm === numCrm);
        }
...
function mostrarProfissionais() {
    ...
    profissionais.forEach(profissional => {
        ...
        nomeUsuario.textContent = profissional.nomeUsuario;
...
function carregarEspecialidadeMedicoCrm() {
    ...
    especialidades.forEach(especialidade => {
        ...
        option.value = especialidade.nomeTipoMedico;
        option.textContent = especialidade.nomeTipoMedico;
